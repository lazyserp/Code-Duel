...
function Lobby()
{
    ...
    async function handleJoin(e)
    {
        ...
        const response = await axios.post("http://localhost:8080/api/matchmaking/join", { userId, difficulty}, { headers:  { Authorization: `Bearer ${token}` }})
        if ( response.data.status == "QUEUED"){
            setStatus("Queued! Waiting for opponent...")

            const intervalId = setInterval(async () => {
                const resp = await axios.get(`http://localhost:8080/api/matchmaking/active?userId=${userId}`, { headers:  { Authorization: `Bearer ${token}` }})
                if (resp.data)
                {
